...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # apply the linear input layer to the query and encoder outputs
        query = self.linear_in(query)
        context = encoder_outputs

        # compute the dot product between the query and context
        attn_scores = torch.bmm(query, context.transpose(1,2))
        
        # apply the mask and perform the softmax
        attn_scores = attn_scores.masked_fill(src_seq_mask.unsqueeze(1), float("-inf"))
        attn_scores = torch.softmax(attn_scores, dim=2)
        
        c = torch.bmm(attn_scores, context)
        attn_out = self.linear_out(torch.cat([query,c], dim=2))
...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        enc_output = unpack(enc_output, batch_first=True)
