...
class CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Convolution 1, ReLU activation, max pooling 1
        x = self.conv1(x)
        #x = self.maxpool1(x)
        x = F.relu(x)
        x = self.maxpool1(x)
        
        # Convolution 2, ReLU activation, max pooling 2
        x = self.conv2(x)
        #x = self.maxpool2(x)
        x = F.relu(x)
        x = self.maxpool2(x)
        ...
        # Affine transformation 1, ReLU activation, dropout
        x = self.fc1(x)
        x = F.relu(x)
        x = self.dropout(x)
        
        # Affine transformation 2, ReLU activation
        x = self.fc2(x)
        x = F.relu(x)
        
        # Affine transformation 3, LogSoftmax output
        x = self.fc3(x)
